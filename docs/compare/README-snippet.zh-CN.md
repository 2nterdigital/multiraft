...
- **不是 Aeron 的 fork** — 开源 Rust Multi-Raft，基于 **openraft `=0.10.0-alpha.30`**，借鉴 Aeron 热路径思路，而非 Media Driver / 商业 Cluster。
- **Standby 范围：** learner standby、复制节流、成员切换和 `read_stale`；`STANDBY=1` 的 catalog/checksum/ad 生成仅限实验室。实时 HTTP/ad/catalog/daisy 恢复已 containment 并返回类型化 unsupported；恢复仍使用正常 OpenRaft recovery。
